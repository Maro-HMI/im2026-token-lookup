...
function init3DVoidModule() {
  ...
  function isTargetKey(key) {
    return key === 'ArrowDown' || key === 'ArrowLeft' || key === 'Down' || key === 'Left';
  }
  ...
  function spawnPressDownImpact() {
    ...
    const rect = pageCard ? pageCard.getBoundingClientRect() : null;
  ...
  window.addEventListener('keydown', (e) => {
    ...
    if (!isTargetKey(e.key)) return;
    if (e.repeat) return; // Prevent OS key-repeat from restarting timer
    if (isInputFocused()) return; // Never trigger while typing in student number
  ...
  function startGameLoop() {
    ...
    function loop() {
      ...
      // Keyboard turn steering if using arrow keys alone
